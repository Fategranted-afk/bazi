...
# === 141. Validating 8-Button Primary Navigation Bar Streamlining & Historical Figures Independent Rendering ===
print("\n=== 141. Validating 8-Button Primary Navigation Bar Streamlining & Historical Figures Independent Rendering ===")

with open("index.html", "r", encoding="utf-8") as f:
    idx_141 = f.read()

# 1. Assert the 8 active primary navigation buttons are visible without 'hidden' in #primaryViewNav
...
visible_8_nav_buttons = [
    "navBtnMasterProfile", "navBtnHome", "navBtnFriction",
    "navBtnLuck", "navBtnCanons", "navBtnSynastry",
    "navBtnCareer", "navBtnHistory"
]

for btn_id in visible_8_nav_buttons:
    ...
    assert 'class="hidden' not in btn_tag and ' hidden ' not in btn_tag, f"Visible button {btn_id} must NOT have hidden class"

# Assert the 4 streamlined buttons have 'hidden'
for btn_id in ["navBtnStrategy", "navBtnIChing", "navBtnSimulator", "navBtnRectification"]:
    assert f'id="{btn_id}"' in pnav_html, f"Streamlined button {btn_id} must exist"
    btn_pos = pnav_html.find(f'id="{btn_id}"')
    btn_tag = pnav_html[btn_pos - 15 : btn_pos + 120]
    assert 'hidden' in btn_tag, f"Streamlined button {btn_id} must have hidden class"
...
# 3. Dynamic headless test of HistoricalEngine 449 figures and Yuwen Tai #1
jsc_check141_cmd = [
    "/System/Library/Frameworks/JavaScriptCore.framework/Versions/Current/Helpers/jsc",
    "-e",
    r"""
